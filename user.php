...
// Fetch users and order them by position: Admin first, then Office Staff, then Site Staff
$query = "SELECT id, fullname, position, profilepicture 
          FROM user 
          ORDER BY FIELD(position, 'admin', 'office staff', 'site staff')";
...
<div class="main">
        ...
        <div class="search-add" style="display: flex; justify-content: center; align-items: center;">
            ...
            <h5 style="margin-bottom: 20px;">List of Users</h5> 
            <form>
                <input type="text" id="searchInput" placeholder="Search User.." onkeyup="searchUser()">
                <button onclick="searchUser()">Search</buttonlog>
            </form>
            </div>
                  <div class="right-section" style="margin-left: 20px;">
                      <a href="add_user.php" class="add-button" style="margin-bottom: 20px;">Add User</a>
                  </div>
          </div>
    ...
    function searchUser() {
        ...
        userCards.forEach(card => {
            let userName = card.querySelector(".user-name").textContent.toLowerCase();
            let userPosition = card.querySelector(".user-position").textContent.toLowerCase();
            
            // Show or hide the user card based on search match
            if (userName.includes(input) || userPosition.includes(input)) {
