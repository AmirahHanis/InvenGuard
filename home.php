...
$pendingRequestQuery = "SELECT RequestID, PICName, ItemID, Amount FROM requestform WHERE RequestStatus = 'Pending'";
$pendingRequestResult = $conn->query($pendingRequestQuery);
?>
...
<div class="main">
    ...
    <div class="container" style="background-color:#f1f1f1">
            ...
            <p style="text-align: left; color: gray; margin-left: 50px;">Home Page</p>
            ...
            <div class="row">
                <div class="column-66">  
                    <div class="store-list">
                        ...
                        <div class="store-card">
                            ...
                            <ul class="stock-list">
                                <?php if ($lowStockResult->num_rows > 0) { ?>
                                    ...
                                    <?php } ?>
                                <?php } else { ?>
                                    <li class="no-entry">No low stock items.</li>
                                <?php } ?>
                            </ul>
                        </div>
                        ...
                        <!-- Pending Requests -->
                        <div class="store-card">
                        <h3><img src="img/wallet.png" alt="Low Stock" class="icon">Request Form : Pending</h3>
                            <ul class="stock-list">
                                <?php if ($pendingRequestResult->num_rows > 0) { ?>
                                    <?php while ($row = $pendingRequestResult->fetch_assoc()) { ?>
                                        <li>
                                            <img src="img/form.png" class="small-icon" alt="Request"> 
                                            <?php echo htmlspecialchars($row['PICName']); ?> requested 
                                            <?php echo htmlspecialchars($row['Amount']); ?> items
                                        </li>
                                    <?php } ?>
                                <?php } else { ?>
                                    <li class="no-entry">No pending requests.</li>
                                <?php } ?>
                            </ul>
                        </div>
